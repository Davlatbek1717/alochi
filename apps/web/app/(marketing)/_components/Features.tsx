import {
  Brain,
  Map,
  Activity,
  Users,
  ScanFace,
  MessageCircle,
  BarChart3,
  Smartphone,
} from 'lucide-react';
import type { ReactNode } from 'react';

interface Feature {
  title: string;
  body: string;
  icon: ReactNode;
  tone: 'violet' | 'orange' | 'green' | 'blue' | 'gold' | 'pink' | 'navy';
}

const TONES: Record<
  Feature['tone'],
  { bg: string; fg: string; ring: string }
> = {
  violet: {
    bg: 'bg-[#6d28d9]/10',
    fg: 'text-[#6d28d9]',
    ring: 'ring-[#6d28d9]/20',
  },
  orange: {
    bg: 'bg-[#f59e0b]/12',
    fg: 'text-[#d97706]',
    ring: 'ring-[#f59e0b]/25',
  },
  green: {
    bg: 'bg-[#15803d]/12',
    fg: 'text-[#15803d]',
    ring: 'ring-[#15803d]/25',
  },
  blue: {
    bg: 'bg-[#1cb0f6]/12',
    fg: 'text-[#0ea5e9]',
    ring: 'ring-[#1cb0f6]/25',
  },
  gold: {
    bg: 'bg-[#fbbf24]/18',
    fg: 'text-[#b45309]',
    ring: 'ring-[#fbbf24]/30',
  },
  pink: {
    bg: 'bg-[#ce82ff]/15',
    fg: 'text-[#a855f7]',
    ring: 'ring-[#ce82ff]/30',
  },
  navy: {
    bg: 'bg-[#1e1b4b]/8',
    fg: 'text-[#1e1b4b]',
    ring: 'ring-[#1e1b4b]/15',
  },
};

const FEATURES: Feature[] = [
  {
    title: 'Adaptiv qiyinlik',
    body: "N-back asosida har o'quvchining darajasiga moslashadi va zerikishni oldini oladi.",
    icon: <Brain size={22} strokeWidth={2.25} />,
    tone: 'violet',
  },
  {
    title: "Yo'l xaritasi",
    body: '250 dars darajama-daraja tartibda — har bir bosqich avvalgisi ustiga quriladi.',
    icon: <Map size={22} strokeWidth={2.25} />,
    tone: 'orange',
  },
  {
    title: 'Real-vaqt holat',
    body: 'Yashil / Sariq / Qizil — Mentor, Manager va Filadmin har soniyada bilib turadi.',
    icon: <Activity size={22} strokeWidth={2.25} />,
    tone: 'green',
  },
  {
    title: 'Ijtimoiy funksiyalar',
    body: "Lenta, do'stlar, duel, group challenges — bola jamoadosh bilan birga o'sadi.",
    icon: <Users size={22} strokeWidth={2.25} />,
    tone: 'blue',
  },
  {
    title: 'Face ID davomat',
    body: "PDPL-mos: faqat 128 o'lchovli vektor saqlanadi, AES-256 bilan shifrlanadi.",
    icon: <ScanFace size={22} strokeWidth={2.25} />,
    tone: 'pink',
  },
  {
    title: 'Telegram bot',
    body: "Ota-onalarga kunlik hisobot, davomat ogohlantirishi va to'lov eslatmalari.",
    icon: <MessageCircle size={22} strokeWidth={2.25} />,
    tone: 'gold',
  },
  {
    title: 'Analitika',
    body: '8 tablik dashboard, ClickHouse asosida real-time, ML churn bashorati bilan.',
    icon: <BarChart3 size={22} strokeWidth={2.25} />,
    tone: 'violet',
  },
  {
    title: 'PWA',
    body: "Telefonga o'rnatish, offline rejimi, push-bildirishnoma — App Store kerak emas.",
    icon: <Smartphone size={22} strokeWidth={2.25} />,
    tone: 'navy',
  },
];

export function Features() {
  return (
    <section
      id="capabilities"
      aria-labelledby="features-h2"
      className="relative bg-[var(--surface)] border-y border-[var(--line)] scroll-mt-20"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-24 sm:py-32">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
          <div className="max-w-2xl">
            <span className="section-eyebrow">Hamma narsa bitta joyda</span>
            <h2
              id="features-h2"
              className="font-display mt-5 text-4xl sm:text-5xl lg:text-6xl font-bold text-[var(--ink)] tracking-[-0.02em] leading-[1.05]"
            >
              Imkoniyatlar
            </h2>
          </div>
          <p className="text-base text-[var(--ink-2)] max-w-md leading-relaxed">
            O&apos;quvchidan superadminacha — har bir rolga zarur asboblar
            yagona kod bazasida.
          </p>
        </div>

        <div className="mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {FEATURES.map((f) => {
            const tone = TONES[f.tone];
            return (
              <article
                key={f.title}
                className={[
                  'group lift relative',
                  'bg-[var(--surface)] rounded-2xl p-6',
                  'border border-[var(--line)]',
                  `ring-1 ${tone.ring}`,
                ].join(' ')}
              >
                <span
                  className={[
                    'grid place-items-center w-12 h-12 rounded-2xl',
                    tone.bg,
                    tone.fg,
                    'transition-transform duration-300 group-hover:scale-110 group-hover:rotate-[-6deg]',
                  ].join(' ')}
                  aria-hidden
                >
                  {f.icon}
                </span>
                <h3 className="mt-5 font-display text-lg font-bold text-[var(--ink)] tracking-[-0.005em]">
                  {f.title}
                </h3>
                <p className="mt-2 text-sm leading-relaxed text-[var(--ink-2)]">
                  {f.body}
                </p>
              </article>
            );
          })}
        </div>
      </div>
    </section>
  );
}
