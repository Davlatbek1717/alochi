...
export class ComponentsService {
  ...
  async setMcq(lessonId: string, questions: McqQuestion[]) {
    await this.prisma.lessonComponent.deleteMany({
      where: { lessonId, type: 'mcq' },
    });
  ...
  async setWordOrder(lessonId: string, sentences: WordOrderSentence[]) {
    await this.prisma.lessonComponent.deleteMany({
      where: { lessonId, type: 'word_order' },
    });
    return this.prisma.lessonComponent.createMany({
  ...
  async setVocabulary(
    lessonId: string,
    words: { uzbek: string; english: string }[],
  ) {
    await this.prisma.lessonComponent.deleteMany({
      where: { lessonId, type: 'vocabulary' },
    });
    return this.prisma.lessonComponent.createMany({
