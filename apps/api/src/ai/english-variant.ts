/**
 * US ↔ UK English spelling equivalence.
 *
 * The platform teaches BOTH American and British English. Free-text answers
 * must be accepted regardless of which variant the student uses (color vs
 * colour, organize vs organise, etc.). This module normalises any English
 * text toward the American form so exact-match graders treat the two
 ...
 * left to the LLM graders, which understand them in context — a blind
 * vocabulary swap here would mis-match words like "lift" used as a verb.
 *
 * Keep this file byte-for-byte in sync with
 * `apps/web/lib/english-variant.ts` (the frontend copy).
 */
 ...
 * Non-English text passes through unchanged (token not in the map).
 */
